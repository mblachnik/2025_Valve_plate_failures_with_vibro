...
def permutation_importance(
    ...
    *,
    ...
    cols_groups: list of lists, wher each element of the main list if a list of feature names for which permutations
        will be appllied. So instead of a single feature now it can be applied to subset of features, where the subset
        is defined by internal list of feature names. Note that X must be pandas dataframe
        .. versionadded:: 1.0
    ...
    #iter = range(X.shape[1]) if cols_groups is None else cols_groups
    scores = Parallel(n_jobs=n_jobs)(
        delayed(_calculate_permutation_scores)(
            ...
            y,
            sample_weight,
            X.columns.get_indexer(col_idx),
            random_seed,
            n_repeats,
            scorer,
            max_samples,
        )
        for col_idx in cols_groups
    )
