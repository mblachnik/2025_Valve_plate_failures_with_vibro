...
for f in fNames:
    ...
    temps = pd.concat(
        [temps, pd.DataFrame(([(df["Temperature - suction line"].min(), df["Temperature - suction line"].max())]))])
...
df0train = pd.concat([dfs[0].iloc[:1529,:], dfs[0].iloc[4417:,:]])
df0test = dfs[0].iloc[1529:4416,:]

#df0train = dfs[0].sample(5708).sort_index()
...
# %%
data_UT1.to_csv(f'data/mb/dataA_1_{percent * 100}.csv', index=False)
data_UT2.to_csv('data/mb/dataA_2.csv', index=False)
data_UT3.to_csv('data/mb/dataA_3.csv', index=False)