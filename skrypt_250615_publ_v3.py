...
#%% Podział danych OT na częci i zbiory treningowe i testowe

num_parts = 69 # Na ile części chcesz podzielić DataFrame
part_size = len(dfs[0]) // num_parts # Oblicza rozmiar każdej części (całkowita liczba wierszy / liczba części)
                            # Operator // zapewnia wynik całkowity

list_of_dfs = []
for i in range(num_parts):
    start_index = i * part_size
    # Upewnij się, że ostatnia część zawiera pozostałe wiersze
    end_index = (i + 1) * part_size if i < num_parts - 1 else len(dfs[0])
    list_of_dfs.append(dfs[0].iloc[start_index:end_index])

df0train=pd.DataFrame()
df0test=pd.DataFrame()

for i in range(0, len(list_of_dfs),3):
    df0train = pd.concat([df0train, list_of_dfs[i]])
    df0train = pd.concat([df0train, list_of_dfs[i+1]])
    # df0train = pd.DataFrame().append(list_of_dfs[i+1])
...
res_df.to_csv('res_df.csv',sep=';')

mean_test_accuracy = np.mean(res_dic_2[0]['mean_test_accuracy'])
